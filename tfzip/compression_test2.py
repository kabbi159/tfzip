...
logits_ = tf.matmul(h_fc2_drop, fc3_pruned) + b_fc3
...
cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits_, labels=y_)
# "Overall, L2 regularization gives the best pruning results."
l2_loss = tf.nn.l2_loss(tf.concat([tf.reshape(W_fc1, [-1]), tf.reshape(W_fc2, [-1]), tf.reshape(W_fc3, [-1])], 0))
...
'''
mean1, variance1 = tf.nn.moments(W_fc1, axes=[0])
threshold1 = tf.square(variance1)
mean2, variance2 = tf.nn.moments(W_fc2, axes=[0])
threshold2 = tf.square(variance2)
mean3, variance3 = tf.nn.moments(W_fc3, axes=[0])
threshold3 = tf.square(variance3)
t1 = tf.sqrt(tf.nn.l2_loss(W_fc1)) * threshold1
t2 = tf.sqrt(tf.nn.l2_loss(W_fc2)) * threshold2
t3 = tf.sqrt(tf.nn.l2_loss(W_fc3)) * threshold3
...
if __name__ == '__main__':
    ...
    #   fc2: 30,000 => 4,196
    #   fc3: 1,000 => 394
    #   total: ~267k => ~18k (~15x compression)
    # Protobuf Size (after gzip):
    #   ~2.8 MB => ~290kB (~10x compression)
